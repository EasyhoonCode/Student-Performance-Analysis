...
class Leftmenu(QWidget):
    ...
    def __init__(self):
        ...
        self.menudata = [{
            "name": "成绩数据看板",
            "type":"Performance_data_Kanban"
        }, {
            "name": "班级成绩表格",
            "type":"Class_grade_form"
        }, {
            "name": "成绩分布统计",
            "type":"student_management"
        }, {
            "name": "数据后台管理",
            "type":"back-stage_management"
        }]
    ...
    def initleftmenu(self):
        ...
        self.leftmenu_group.setStyleSheet("background-color:{};border-radius:10px;".format(static.data["leftmenu"]["bg"]))
    ...
    def initmenu(self):
        ...
        for index, item in enumerate(self.menudata):
            ...
            #self.leftmenu_menu_bg.setStyleSheet("border-radius:10px;")
            ...
            self.leftmenu_menu_icon.setStyleSheet(
                "background:url({}) no-repeat center center;width: 20px;height: 20px;".format(static.data["leftmenu_menu_icon"][item["type"]]))
            self.leftmenu_menu_label = menuframelabel(item["name"])
            self.leftmenu_menu_label.setStyleSheet("color: #ffffff;font-size:15px;font: bold;".format(static.data["leftmenu_menu_label"]["color"]),)
    ...
    def clicksetmenuframebg(self, event, index):
        for i in range(self.menus_QVlayout.count()):
            if i == index:
                ...
                self.menus_QVlayout.itemAt(i).widget().setStyleSheet("background-color: {};border-radius:10px;".format(static.data["leftmenu_menu_bg"]["focusbackground"])  )
...
class menuframe(QFrame):
    ...
    def enterEvent(self, event):
        self.setStyleSheet("background-color: {};border-radius:10px;".format(static.data["leftmenu_menu_bg"]["focusbackground"]))
